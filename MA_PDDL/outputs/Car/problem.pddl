...
(:types agent - object )
(:predicates (running ?a - agent )(engineblown ?a - agent )(transmission_fine ?a - agent )(goal_reached ?a - agent ))
...
(:action accelerate :parameters (?a - agent ):precondition (and (running ?a )(< (a ?a )(up_limit ?a ))):effect (and (increase (a ?a )1 )))
(:action decelerate :parameters (?a - agent ):precondition (and (running ?a )(> (a ?a )(down_limit ?a ))):effect (and (decrease (a ?a )1 )))
...
(:action stop :parameters (?a - agent ):precondition (and (= (v ?a )0 )(>= (d ?a )30 )(not (engineblown ?a ))):effect (goal_reached ?a ))
)