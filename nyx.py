...
def runner(dom_file, prob_file, args_list: []):
    ...
    if not my_plnr.reached_goal_states:
        ...
        if (constants.TRACK_VARIABLES or constants.PLOT_TRACKED_VARIABLES) and constants.TRACKED_VARIABLES_LIST == []:
            ...
            constants.TRACKED_VARIABLES_LIST = [s.replace("[", "").replace("]", "").replace("'", "").replace(",", "")
        ...
        for sg in my_plnr.reached_goal_states:
            ...
            if constants.TRACK_VARIABLES or constants.PLOT_TRACKED_VARIABLES:
                extract_variable_values(one_plan_file, constants.TRACKED_VARIABLES_LIST,
...
def process_goal(plan_obj, planner_obj, plan_save_file):
    ...
    for pair in plan_obj:
        ...
        if (not (constants.VERBOSE_OUTPUT or constants.VERY_VERBOSE_OUTPUT)) and pair[
            0] == constants.TIME_PASSING_ACTION:
            continue

        str1 = '' + str(
            "{:10.3f}".format(planner_obj.visited_hashmap[pair[1].predecessor_hashed].state.time)) + ':\t' + str(
            pair[0].name)
...
if __name__ == '__main__':
    ...
    runner('MA-PDDL/outputs/WithObjectsConversion/Blocks/domain_4_agents.pddl',
           'MA-PDDL/outputs/WithObjectsConversion/Blocks/problem_4_agents.pddl', ['-t:1'])
